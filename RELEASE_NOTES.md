...
| v0.8.20 | 2026-07-13 | 코딩 모드 수정 루프 재귀 오염 차단: v0.8.19 가 막지 않은 "실패 원문 하류 전달" 구멍으로 Claude 타임아웃 문구가 테스트 결과 -> Codex 가짜 이슈 -> Claude 산문 -> `current_code` 로 이어지던 사슬 절단. Phase 1/2/3·수정 루프·병렬 모드·최종 보고서 전 구간에 `needs_replacement` 가드 |
| v0.8.21 | 2026-07-13 | 타임아웃 정책 전면 수정 (v0.8.20 오염의 상류 원인). "574초" 는 stale 값이고 실제 634초였다: 읽기 루프가 elapsed 를 루프 진입 시점에 재고 readline 을 고정 60초 기다린 뒤 그 낡은 값을 보고 + 한도를 60초 초과. 숨은 `t*2` 배수 제거(에이전트별 실효 예산 불일치 해소), Claude 외부 가드 신설, 타임아웃 kill 사정거리를 "호출 단위" 로 축소(형제 에이전트 동반 사망 차단), 가드 취소 시 kill→정리 순서 교정(부산물 누수) |

## v0.8.21 (2026-07-13)

v0.8.20 은 "실패 원문이 하류로 흐르는" 증상을 막았다. 이번엔 그 실패 자체를 만든 **상류 원인**, 즉 타임아웃 정책을 고친다. 실측 사고는 코딩 모드에서 Claude 가 `[Claude] 응답 대기 시간 초과 (574초)` 와 `[Claude] 응답 시간 초과 (300초)` 를 낸 것이다.

### 근본 원인

- **[Major / backend] R1 - "574초" 는 거짓 숫자였다 (stale elapsed + 데드라인 초과)**
  `agents/claude.py`(및 동형인 `agents/gemini.py`)의 읽기 루프는 `elapsed` 를 **루프 진입 시점**에 재고, `readline` 을 **고정 60초** 기다린 뒤, 타임아웃 메시지에 그 **낡은 값**을 그대로 찍었다.
  전체 한도 600초에 574초 시점으로 진입 → `574 > 600` 이 아니므로 통과 → readline 60초 대기 → 실제 634초에 종료되면서 `int(elapsed)` = **574** 로 보고. 즉 (a) 보고 숫자가 60초 어긋나고 (b) 한도를 `readline_timeout` 만큼 넘긴다(실효 상한 660초).
  수정: readline 대기를 `min(STREAM_IDLE_TIMEOUT, 남은 예산)` 으로 자르고, 보고 직전 `elapsed` 를 다시 잰다. 메시지도 `응답 시간 초과 (634초 / 한도 600초)` 로 한도를 함께 밝힌다.

- **[Major / backend] R2 - 같은 `timeout=` 이 에이전트마다 다른 예산을 뜻했다**
  `base`(Codex)는 `t`, Claude 는 `t*2`(+60), Gemini 는 `t*2`(외부 가드 `t*2.5`). 같은 `timeout=300` 을 넘겨도 Codex 는 300초, Claude 는 660초까지 갔다. 한 실행에 "300초" 와 "574초" 가 같이 찍힌 이유가 이것이다.
  수정: 숨은 배수를 없애고 모드별 예산을 상수로 못박았다. `CLI_TIMEOUT` 360, `CLI_TIMEOUT_CODING` 600, `CLI_TIMEOUT_RESEARCH` 180(신설).
  실효 예산 변화(의도적 조정이며 "유지" 가 아니다): 코딩 Claude 660→600초(축소), 코딩 Codex 300→600초(**확대**). Codex 확대는 의도다 - 예전엔 Codex 만 절반 예산이라 정상적인 장시간 테스트 작성 중 혼자 먼저 타임아웃 나 불필요하게 백업으로 교체됐다. 토론은 Claude 420→360초, Codex 180→360초이고, 라운드는 `gather` 병렬이라 라운드 체감 상한은 420→360초로 오히려 줄어든다.

- **[Major / backend] R3 - 타임아웃 정리가 형제 에이전트를 동반 사망시켰다**
  `_kill_registered_processes()` 가 `thread_ts` 에 등록된 **모든** 프로세스를 죽였다. 그런데 모든 에이전트는 같은 `_current_thread_ts` 를 공유하며 `asyncio.gather` 로 병렬 실행된다(코딩 Phase 3, 리서치 분담 조사). 게다가 리서치는 하위질문을 라운드로빈 배정하므로 **같은 인스턴스가 동시에 2건**을 맡기도 한다(4문항/3에이전트).
  즉 한 에이전트의 타임아웃이 멀쩡히 돌던 다른 에이전트(및 같은 인스턴스의 다른 호출)의 CLI 프로세스를 죽였고, 그 결과가 "테스트 결과 0건 → 가짜 이슈" 였다.
  수정: `contextvars.ContextVar` + `AgentBase._call_scope()` 로 kill 사정거리를 **호출 단위**로 좁혔다. 사용자 취소(`/cancel`)는 기존대로 스레드 전체를 죽인다(의도된 동작).

- **[Major / backend] R4 - Claude 에 외부 가드가 없었다**
  읽기 루프 **밖**의 await(`stdin.drain()`, `proc.wait()`)은 내부 데드라인 검사에 도달하지 못해 영구 hang 이 가능하다(v0.7.3.2 Gemini 33분 hang 과 같은 계열). Gemini 만 가드를 받았고 Claude 는 무방비였다.
  수정: `base.ask_with_progress` 가 `_stream_once` 를 `t * STREAM_GUARD_FACTOR`(1.25) 로 감싼다. Claude 는 `_stream_once` 만 오버라이드하도록 정리했고, Codex 는 `super()` 위임이라 자동으로 가드를 받는다. 상한은 "정상 t, 병리적 hang 시 t*1.25" 다.

### Codex 교차검증 반영

- **[Major] 가드 취소 시 부산물 누수 (Codex 지적, 수정)**: `asyncio.wait_for` 는 내부 Task 를 cancel 하고 **그 Task 의 `finally` 가 끝난 뒤**에야 호출부 `except` 로 넘어간다. 따라서 기존 순서로는 CLI 가 **아직 살아있는 채로** `-o <tmp>.last.md` 부산물과 프롬프트 파일을 지웠고, 살아있는 writer 가 삭제 직후 파일을 다시 써서 임시 폴더에 남았다. `_kill_if_alive()` 를 파일 정리 **앞**에 두어 writer 를 먼저 죽인다(동기 함수라 cancel 중인 `finally` 에서도 안전). 실제 writer 가 파일을 다시 쓰는 상황을 재현하는 회귀 테스트 추가.
- **[Major] 데드라인이 spawn/drain 을 포함하지 않았다 (Codex 지적, 수정)**: `start_time` 을 spawn **뒤**에 잡아서, Windows 의 `.cmd` 래퍼 → node 부팅 시간과 stdin drain 이 예산에 안 잡혔다. `start_time` 을 spawn 전으로 옮겼다.
- **[Major] "실효 예산 유지" 주장 정정 (Codex 지적, 문서 수정)**: Codex 예산은 실제로 300→600초(코딩), 180→360초(토론)로 **늘었다**. 주석/노트에서 "유지" 주장을 걷어내고 의도적 조정임을 명시했다.
- **[알려진 잔여 이슈, 미수정]** `self.timed_out` / `self.has_error` / `last_usage` 는 여전히 인스턴스 공유다. 리서치가 같은 인스턴스를 병렬 호출하므로 이론상 플래그 경합이 가능하다. 현재 호출부는 `ask()` 반환 직후 await 없이 곧바로 플래그를 읽어 실제 트리거는 없으나(이벤트 루프가 그 사이에 다른 코루틴을 끼워넣지 못함), 구조적으로 취약하다. 별도 이슈로 분리해 추적한다.

### 재시도 정책 (점검 결과)

타임아웃은 **재시도하지 않는다**. 이미 예산을 다 쓴 호출을 같은 예산으로 다시 돌리면 실패까지 걸리는 시간만 2배가 된다. 타임아웃은 곧바로 백업 교체로 넘긴다. 다만 **백업이 primary 와 같은 예산을 받도록** 고쳤다(`modes/coding.py` 의 백업 호출이 `timeout` 인자를 빼먹어 코딩 예산 600초가 아닌 기본값으로 줄어들고 있었다 - primary 가 못 끝낸 일을 더 짧은 예산으로 시키는 꼴). 재시도는 transient 인프라 에러(5xx/429)에 한해 최대 1회만 유지한다(토론 합의 경로, Gemini 429 백오프).

### 검증

- 단위 테스트 486건 통과 (신규 `tests/test_timeout_policy_v0821.py` 7건 포함, 전부 사전에 red 확인)
- 실제 subprocess 로 구동 검증: hard wall 준수(예산 5초 → 5.2초 종료, 예전엔 65초), 보고 숫자 정직성, 자식 프로세스 실제 kill, 형제 프로세스 생존, `/cancel` 의 스레드 전체 kill 유지

## v0.8.20 (2026-07-13)

v0.8.19 P1 은 실패 응답 원문의 **게시**만 막았고 **전달**은 막지 않았다. 그 구멍으로 코딩 모드의 수정 루프가 자기 자신을 오염시키는 재귀가 실측으로 관측됐다.

### 재귀 오염 사슬 (실측)

1. Phase 3 에서 Claude 타임아웃 -> `claude_tests = "[Claude] 응답 대기 시간 초과 (574초 무응답)"`
2. v0.8.19 는 이걸 게시하지 않았을 뿐, `all_tests` 로 이어붙여 Codex 에 넘겼다 (`coding.py:871`).
3. Codex 가 이 타임아웃 문구를 **테스트 결과로** 읽고 "이슈 있음: 타임아웃으로 테스트가 완료되지 않아 수정 필요 여부를 판단할 수 없습니다" 를 이슈로 보고했다.
4. 그 **가짜 이슈** + 코드가 Claude 에게 수정 요청으로 나갔다 (`coding.py:893`).
5. Claude 는 고칠 코드 결함이 없으니 "이건 코드 이슈가 아닙니다" 라는 **산문**을 반환했다.
6. 그 산문이 `current_code` 에 대입돼 `*[수정된 코드]*` 로 게시되고, 다음 라운드에 `기존 코드` 로 재투입됐다. 코드 산출물이 산문으로 대체되는 재귀.

근본 원인은 하나다: **`BaseAgent` 가 실패를 예외가 아니라 문자열로 반환하는데, 코딩 모드의 하류 소비자들이 그 문자열을 정상 내용으로 취급했다.**

### 원인과 수정 (`modes/coding.py`)

- **[Critical / backend] 실패 문구가 이슈 판단 근거로 유입** (`_run_review_and_test`)
  - Phase 3 슬롯이 실패하면 결과를 `None` 으로 폐기하고, 성공한 것만 `all_tests` 로 합친다. 실패 원문은 게시뿐 아니라 하류 프롬프트로도 나가지 않는다.
  - 테스트 결과가 0건이면 이슈 판단 자체를 건너뛴다. 근거 없이 물으면 "판단 불가" 가 이슈로 돌아온다.

- **[Critical / backend] 수정 루프가 자기 실패로 코드를 덮어씀** (`_run_review_and_test`)
  - Codex 이슈 판단이 실패하면(`needs_replacement`) 그 원문을 이슈로 게시하거나 수정 프롬프트에 넣지 않고 루프를 중단한다.
  - Claude 수정 호출이 실패하면 `current_code` 를 실패 문자열로 덮어쓰지 않고 직전 코드를 유지한 채 중단한다.

- **[Critical / backend] Phase 1 실패 문구가 "코드" 로 리뷰/테스트됨** (Codex 교차검증)
  - 주/백업이 모두 실패하면 `_ask_with_backup` 의 반환 문구가 `claude_code` 가 되어 Phase 2 리뷰 프롬프트와 Phase 3 테스트 프롬프트에 코드로 삽입됐다. 이제 Phase 1 실패 시 후속 단계로 진입하지 않는다.

- **[Major / backend] 백업 풀 소진 시 실패 원문이 답변으로 반환** (`_ask_with_backup`)
  - 백업이 없으면 실패 원문을 그대로 돌려줘 호출부가 그걸 답변으로 게시하고(v0.8.19 정책 위반) 하류로도 흘렸다. 정제 문구를 반환하고 `needs_replacement` 로 실패를 알린다.

- **[Major / backend] 병렬 모드 실패 원문이 요약 프롬프트로 유입** (`_parallel_start`, Codex 교차검증)
  - 백업이 없을 때 실패 원문이 `final_responses` 에 그대로 담겨 최종 요약 프롬프트로 갔다. 결과 없이 제외하도록 수정. 전원 실패면 빈 요약을 요청하지 않는다.

- **[Major / backend] 보고서 생성 실패 원문이 최종 보고서로 방송** (양쪽 모드, Codex 교차검증)
  - 보고서 호출이 실패하면 그 문구가 `📋 최종 보고서` 로 방송됐다. 실패 고지로 대체한다.

- **[Minor / backend] 빈 정상 응답을 실패로 오인**
  - `None`(호출 실패) 과 `""`(내용 없는 정상 응답) 을 구분하는 `_test_summary()` 도입. 보고서에 "(응답 실패)" 와 "(빈 응답)" 을 다르게 적는다.

### 테스트
- 신규 8건: `tests/test_coding_fix_loop_v0820.py`(4, 재귀 오염 사슬), `tests/test_coding_failure_leak_v0820.py`(4, Codex 교차검증 지적분). 전체 478건 통과.

### 알려진 한계 (후속)
- **애초에 왜 574초 타임아웃이 났는가**는 별개 문제로 미해결. `agents/claude.py` 의 readline/전체 데드라인 계산과 에이전트별 `timeout=` 의미 불일치(base=t, Claude=t*2)를 따로 봐야 한다. 이번 수정은 "타임아웃이 났을 때 파이프라인이 가짜 이슈를 만들어내지 않게" 하는 데까지다.
